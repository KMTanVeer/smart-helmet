 ...
 * ------------------------
 * - Power-on melody when system connects
 * - 10-second warning tone on crash detection
 * - 3-second beep pattern before SMS
 * - SMS can be aborted during beep pattern by pressing button
 * 
 * HARDWARE COMPONENTS:
 * --------------------
 ...
 * 5. PASSIVE Buzzer (Alert) - Can produce different tones
 ...
 * OPERATION:
 * ----------
 * 1. System initializes and plays power-on melody
 * 2. Continuously monitors acceleration and rotation
 * 3. Detects crash when both thresholds exceeded for >30ms
 * 4. Plays 10-second warning tone
 * 5. Plays 3-second beep pattern (can be cancelled with button)
 * 6. Sends SMS with Google Maps location link (if not cancelled)
 * 
...
const char PHONE_NUMBER[] = "+8801758161680";   // ⚠️ CHANGE TO YOUR EMERGENCY CONTACT

/* ================= PASSIVE BUZZER & BUTTON ================= */
#define BUZZER_PIN 25   // Passive buzzer for alert
...
#define ACC_THRESHOLD   1.2     // g-force (1g = normal gravity, 2.5g = significant impact)
#define GYRO_THRESHOLD  80.0   // degrees per second (rapid rotation indicates fall)
...
bool smsAborted = false;     // Flag: SMS sending was aborted by user
...
unsigned long beepPatternStart = 0;
...
// Plays power-on melody when system starts
void playPowerOnTone() {
  Serial.println("🔊 Playing power-on melody...");
  ...
  delay(350);
  noTone(BUZZER_PIN);
  Serial.println("✅ Power-on melody completed");
}

// Starts 10-second warning tone (continuous)
void startWarningTone() {
  Serial.println("🚨 Starting 10-second warning tone...");
  buzzerState = BUZZER_WARNING_TONE;
  warningToneStart = millis();
  tone(BUZZER_PIN, NOTE_A5);  // Continuous 880 Hz tone
}

// Stops warning tone and starts beep pattern
void startBeepPattern() {
  Serial.println("🔔 Starting 3-second beep pattern...");
  noTone(BUZZER_PIN);
  buzzerState = BUZZER_BEEP_PATTERN;
  beepPatternStart = millis();
}

// Stops all buzzer sounds
void stopBuzzer() {
  noTone(BUZZER_PIN);
...
// Handles beep pattern during SMS preparation
...
int readBatteryPercent() {
  ...
  float vAdc = adc * (ADC_REF / ADC_MAX);

  // 100k / 47k divider
  float batteryVoltage = vAdc * (147.0 / 47.0);

  int percent = map(batteryVoltage * 100, 330, 420, 0, 100);
...
void setup() {
  ...
  delay(1000);
    ...
    // ✅ ESP32 ADC configuration (PLACE HERE)
  analogReadResolution(12);        // 0–4095
  analogSetAttenuation(ADC_11db);  // allows up to ~3.6V

  Serial.println("\n\n========================================");
  Serial.println("   SMART HELMET - PASSIVE BUZZER       ");
  Serial.println("========================================\n");
  ...
  // Test SIM800L connectivity and play power-on tone if ready
  if (sim800Ready()) {
    Serial.println("✅ SIM800L ready and registered on network");
    playPowerOnTone();  // Play melody when system is ready
  } else {
...
void loop() {
  ...
  if (digitalRead(CANCEL_BTN) == LOW) {
    // If in beep pattern phase, abort SMS
    if (buzzerState == BUZZER_BEEP_PATTERN) {
      Serial.println("❌ SMS ABORTED BY USER DURING BEEP PATTERN");
      smsAborted = true;
    }
  ...
  // ===== BUZZER STATE MACHINE =====
  // Handle warning tone timing (10 seconds)
  if (buzzerState == BUZZER_WARNING_TONE) {
    if (millis() - warningToneStart >= WARNING_TONE_DURATION) {
      startBeepPattern();
    }
  }
  
  // Handle beep pattern timing (3 seconds)
  if (buzzerState == BUZZER_BEEP_PATTERN) {
    unsigned long beepElapsed = millis() - beepPatternStart;
    
    if (beepElapsed >= BEEP_PATTERN_DURATION) {
      // Beep pattern complete - proceed to send SMS if not aborted
      stopBuzzer();
      
      if (!smsAborted && gps.location.isValid() && !smsSent) {
        Serial.println("📍 Beep pattern complete - sending emergency SMS...");
  ...
  if (accMag > ACC_THRESHOLD && gyroMag > GYRO_THRESHOLD) {
    ...
    if (millis() - impactStart > IMPACT_TIME_MS) {
      if (!crashDetected) {
        crashDetected = true;
        smsAborted = false;  // Reset abort flag
  ...
  // 🔋 Battery percentage update (every 2 seconds)
