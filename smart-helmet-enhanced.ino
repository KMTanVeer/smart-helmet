...
/* ================= THRESHOLDS ================= */
#define ACC_THRESHOLD   2.5     // g-force
#define GYRO_THRESHOLD  120.0   // degrees per second
#define IMPACT_TIME_MS  30      // milliseconds
...
int readBatteryPercent() {
  ...
  // Multiply by voltage divider ratio to get actual battery voltage
...
void initCrashLogging() {
  ...
  if (!file) {
    ...
    if (file) {
      // Write CSV header
      file.println("Timestamp,Latitude,Longitude,AccMag,GyroMag,Battery%");
      file.close();
    }
  } else {
